...
public class MovieDao extends AbstractMFlixDao {
  ...
  private boolean validIdValue(String movieId) {
    ...
    return movieId.matches("-?[0-9a-fA-F]+")?true:false;
  }
  ...
  public Document getMovie(String movieId) {
    ...
    Bson matchComments = Aggregates.match(Filters.ne("comments", Arrays.asList()));
    Bson lookup = Aggregates.lookup("comments","_id", "movie_id","comments");
    pipeline.add(lookup);
    pipeline.add(matchComments);
  ...
  public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
    Bson castFilter = Filters.and(Filters.exists("cast"), Filters.in("cast", cast));
    Bson sort = new Document(sortKey, 1);
